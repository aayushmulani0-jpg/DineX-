          ...
          {tables.map((table) => {
            ...
            return (
              <div key={table.id} className="table-box">
                ...
                <h4 className="table-total">Total: ₹{tableTotal.toFixed(2)}</h4>
...
//       try {
//         const billsRes = await axios.get("http://localhost:5000/api/bills");
//         const totalRevenue = billsRes.data.reduce((sum, bill) => sum + bill.amount, 0);
//         setRevenue(totalRevenue);
//         setBillCount(billsRes.data.length);
...
//         const today = new Date();
//         today.setHours(0, 0, 0, 0);
//         const todayBills = billsRes.data.filter(bill =>
//           new Date(bill.createdAt) >= today
//         );
//         const todayTotal = todayBills.reduce((sum, bill) => sum + bill.amount, 0);
//         setTodayRevenue(todayTotal);
//       } catch (error) {
//         console.error("Failed to fetch stats:", error);
//       }
...
//   useEffect(() => {
//     // Filter out orders that are not truly active (draft status or no items)
//     const validActiveOrders = activeOrders.filter(order =>
//       order &&
//       order.items &&
//       order.items.length > 0 &&
//       order.status !== "draft"
...
//             View Analytics
//           </Link>
//           <Link to="/billing" className="menu-item hero-link-secondary">
//             Billing History
//           </Link>
//         </div>
//       </div>
...
//           <div className="table-card-actions">
//             <div className="order-count">
//               {filteredActiveOrders.length} order{filteredActiveOrders.length !== 1 ? 's' : ''}
//             </div>
//             {addNewTable && (
//               <Button
//                 type="primary"
...
//         <div className="table-grid">
//           {tables.map((table) => {
//             const order = filteredActiveOrders.find((o) => o.tableId === table.id);
//             const isOccupied = !!order;
